...
# Adapted by Thomas Roder
# Email: roder.thomas@gmail.com
...
def run(args):
    ...
    unique_alignments = calculateUniqueness(lines_by_query, unique_length, keep_small_uniques)
...
def calculateUniqueness(lines_by_query, unique_length, keep_small_uniques):
    unique_alignments = {}
...
def summarize_planesweep(lines, unique_length_required, keep_small_uniques=False):
    ...
    if len(lines) == 1:
        if keep_small_uniques is True or abs(lines[0][1] - lines[0][0]) >= unique_length_required:
            return [0]
        else:
            return []
    ...
    for query_min, query_max in lines:
        ...
        if sorted_unique_intervals_left[i] == query_min and sorted_unique_intervals_right[i] == query_max:
            exact_match = True
        sum_uniq = 0
        while i < len(sorted_unique_intervals_left) \
